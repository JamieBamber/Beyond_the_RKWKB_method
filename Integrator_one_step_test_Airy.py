...
t01 = np.array([t_start1])
x01 = Eq["true_sol"](t01)
x01 = x01.reshape(x01.size, 1)

t02 = np.array([t_start2])
x02 = Eq["true_sol"](t02)
x02 = x02.reshape(x02.size, 1)

n_steps = 500

t_vec1 = np.linspace(t_start1,t_stop1, n_steps)
t_vec2 = np.linspace(t_start2,t_stop2, n_steps)

t_vec0 = np.linspace(t_start1, t_stop2, 1000)
	
def Integrator(t_vec, x0, M):
	T_0 = time.time()
	"""
...
for data in lines:
	print(data["fname"] + ": ")
	data["x1"] = Integrator(t_vec1, x01, data["M"])
	data["x2"] = Integrator(t_vec2, x02, data["M"])
	
linenames = ""
for data in lines:
	linenames = linenames + data["fname"] + "_"

filename = "Analytic_test_" + Eq["name"] + "_comb_" + linenames
filename = filename + "index=" + str(Index)
title = Eq["title"] + " : comparing WKB vs Magnus" # $x_i$ index = " + str(Index)
...
for i in range(len(lines)):
	M = lines[i]
	ax0.plot(t_vec1, M["x1"][:,Index], colours[i]+linetypes[i], markersize=4, linewidth=line_width, label=M["name"])
ax0.plot(t_vec0, x_true[:,Index], color="0.5", linewidth=line_width, linestyle="--", label="true soln.")

ax1 = plt.subplot(gs[1], sharex = ax0)

for i in range(len(lines)):
	M = lines[i]
	ax1.plot(t_vec2, M["x2"][:,Index], colours[i]+linetypes[i], markersize=4, linewidth=line_width)
ax1.plot(t_vec0, x_true[:,Index], color="0.5", linewidth=line_width, linestyle="--")

ax1.set_xlabel("$t$", fontsize=label_size, labelpad=-0)
ax0.set_ylabel("$x$", fontsize=label_size, labelpad=-5)
ax1.set_ylabel("$x$", fontsize=label_size, labelpad=-5)
ax0.set_xlim(0, 45)
ax0.set_ylim(-1, 1)
ax1.set_ylim(-1, 1)
...
ax0.legend(fontsize=legend_font_size, ncol = 2, bbox_to_anchor=(0.30, -0.25, 0.45, 0.25), shadow=False)
plt.subplots_adjust(hspace=.0)
plt.subplots_adjust(left=0.12, right=0.97, top=0.92, bottom=0.13)
