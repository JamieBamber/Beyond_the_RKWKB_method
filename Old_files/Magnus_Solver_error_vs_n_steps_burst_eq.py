...
A = A_burst # choose the A(t) matrix
...
x_true = burst_soln(n_burst, n_burst)
...
for N in [1, 2]:#, 3]:
	for i in range(n_.size):
		t_vec = np.linspace(-n_burst, n_burst, n_[i])
		x = Integrator_1(t_vec, x0, N)
		x_[i, N-1] = x[-1, 0]
		print('#',i)
	
err = np.abs((x_true*np.ones(x_.shape) - x_)/x_true)
...
######## Save Results #################
'''
filename = "Burst_error_vs_n_steps"
...
f.write("n_steps	err (1st ord.)	err (2nd ord.)  \n")
for i in range(n_.size):
	f.write("{}	{}	{}\n".format(n_[i], err[i, 0], err[i, 1]))
f.close()
...
'''
f = open(file_path, "r")
err = np.loadtxt(f, skiprows=4, delimiter='	', usecols=(1, 2))
print("read data")
...
#plt.plot(np.log(n_), np.log(err[:,2]), 'g-', linewidth=1, label="3rd order")
plt.xlabel("no. steps")
#plt.xlabel("log$_{10}$(no. steps)")
plt.ylabel("log$_{10}$(% err at t =" + str(n_burst) + ")")
plt.xscale('log')
plt.ylim(-20,20)
plt.minorticks_on()
plt.title(filename)
