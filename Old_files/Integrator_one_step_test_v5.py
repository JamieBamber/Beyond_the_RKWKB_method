...
PCJM1l = {
	"name" : "const. P Jordan-Magnus sol. (with $\\Lambda$ only)",
	"fname" : "PCJMl",
	"order" : 2,
	"analytic" : True 
}
...
PCJM1lk = {
	"name" : "const. P Jordan-Magnus sol. (with $\\Lambda$ and $K$)",
	"fname" : "PCJMlk",
	"order" : 2,
	"analytic" : True
}
...
def Pconst_Jordan_Magnus(Lambda_only, Use_symbolic):
	#
	global started_PCJM	# have we started integrating?
	global count_PCJM	# how many times have we recalculated P?
	global P_PCJM 		# current P estimate
	global Mf_PCJM		# current Mf
	#
	def get_new_P(tp):
		A_t_num = A_num(tp)
		...
		P_num = sym.lambdify((), P, modules=array2mat_c)()
	...
	def Make_M_from_new_P(tp):
		...
		Pinv = P.inv()
		P_num = sym.lambdify((), P, modules=array2mat_c)()
		#P_num = sym.matrix2numpy(P, dtype=complex)
		P_PCJM = P_num/np.linalg.norm(P_num, ord='fro')	# store the normalised P_num
		Pinv_num = np.linalg.inv(P_num)
		if Use_symbolic:
			A = A_sym.subs(ts, ts0)
			LK = Pinv @ A @ P 
			#print("LK = ", LK)
			...
			#print("Ω1 = ", Om1)
			#print()
			#print( sym.simplify(sym.nsimplify(P @ sym.exp(Om1) @ Pinv)) )
			Om1_num = sym.lambdify((ts0, ts), Om1, modules=array2mat_c)
		elif not Use_symbolic:
			def LK_num(t):
				LK_t = Pinv_num @ A_num(t) @ P_num
				if Lambda_only:
					LK_t = np.multiply(LK_t, np.identity(Ndim))
				return LK_t
			def Omega_num(t0, t):
			...
			Om1_num = Omega_num
		def Mf(t0, t):
			M_ = P_num @ linalg.expm(Om1_num(t0, t)) @ Pinv_num
			return M_ #.astype(np.complex128)
		return Mf
	#
	started_PCJM = False
	...
	def M_func_adaptive(t0, t):
		global started_PCJM, count_PCJM, P_PCJM, Mf_PCJM 	
		t_mid = t
		# get first P matrix at t=t0
		if not started_PCJM:
			Mf_PCJM = Make_M_from_new_P(t_mid)
			started_PCJM = True
		...
		if count > count_PCJM:
			...
			P_new = get_new_P(t_mid)
			dP_norm = np.linalg.norm(P_new - P_PCJM, ord='fro')
			#print("  count = " + str(count) + ", dP_norm = ", dP_norm)
			if dP_norm > 0.01:
				print(" making new P estimate, t_mid = ", t_mid)
				P_PCJM = P_new
				Mf_PCJM = Make_M_from_new_P(t_mid)
				texamp = 0.5*(Eq["t_stop"] - Eq["t_start"])
				#print(Mf_PCJM(texamp, 1.1*texamp))
				#print(P_PCJM)
		M_ = Mf_PCJM(t0, t)
		return M_
	return M_func_adaptive
	
def Pconst_Jordan_Magnus_L():
	return Pconst_Jordan_Magnus(True, PCJM1l["analytic"])
	
def Pconst_Jordan_Magnus_LK():
	return Pconst_Jordan_Magnus(False, PCJM1lk["analytic"])
...
PCJM1l["Mfunc"] 	= Pconst_Jordan_Magnus_L
PCJM1lk["Mfunc"] 	= Pconst_Jordan_Magnus_LK
