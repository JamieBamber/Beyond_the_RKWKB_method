...
def Magnus_Integrator(t_start, t_stop, h0, x0, A, alpha, order):
	...
	#
	def M(time, hstep):
		return linalg.expm(Omega(time, time+hstep, A, alpha, order))
	#
	while t <= t_stop:
		x_n = x_[n,:].reshape(Ndim, 1)
		Err_small = False
		...
		while Err_small == False:
			...
			Err_max = epsilon*np.abs(x_np1_l[0,0] + h*(A(t) @ x_n)[0,0])		# maximum error allowed
			#
			if Err <= Err_max:
				h_new = h*S*np.power(np.abs(Err_max/Err), 1.0/(order + 1))	
				Err_small = True # break loop
			elif Err > Err_max:
				h_new = h*S*np.power(np.abs(Err_max/Err), 1.0/(order))
				if h_new < h_min:
					h_new = h_min
					Err_small = True # break loop
				elif h_new >= h_min:
...
def plot_graph():
	...
	######## Integration ##################
	M4_GL["data"] = Magnus_Integrator(Eq["t_start"], Eq["t_stop"], h0, Eq["x0"], Eq["A"], alpha_GL, 4)
	M4_D["data"] = Magnus_Integrator(Eq["t_start"], Eq["t_stop"], h0, Eq["x0"], Eq["A"], alpha_D, 4)
	#M4_SNC["data"] = Magnus_Integrator(Eq["t_start"], Eq["t_stop"], h0, Eq["x0"], Eq["A"], alpha_SNC, 4)
	#M6_GL["data"] = Magnus_Integrator(Eq["t_start"], Eq["t_stop"], h0, Eq["x0"], Eq["A"], alpha_GL, 6)
	#M6_SNC["data"] = Magnus_Integrator(Eq["t_start"], Eq["t_stop"], h0, Eq["x0"], Eq["A"], alpha_SNC, 6)
	...
	ax2 = ax0.twinx()
	ax0.plot(t_vec0, x_true[:,0], color="0.5", linewidth=1, linestyle="--", label="true soln.")
	for i in range(0, len(lines)):
		...
		T = line["data"][2]
		ax0.plot(t, x, colours[i] + markertypes[i], markersize=1, linewidth=1, label="{:s}, T={:.4g}s".format(line["name"], T))
		ax2.plot(t, np.abs((x - x_true)/x_true), colours[i] + '--', linewidth=2, alpha=0.2, label="{:s} error".format(line["name"]))
	ax0.set_xlabel("t")
	ax2.set_ylabel("rel. error")
	ax2.set_yscale("log")
	ax0.set_ylabel("x")
	ax0.set_ylim(-1, 1)
	plt.title(Eq["name"] + " equation, adaptive step size")
	lgnd = ax0.legend(fontsize=8)
