...
dG = sym.diff(G, ts)
ddG = sym.diff(dG, ts)
A_sym_doublet = sym.Matrix([[0, 1], [ddG/G - dF**2 - 2*(dG/G)**2 - (dG*ddF)/(G*dF), ddF/F - 2*dG/G]])
...
def Jordan_WKB():
	...
	if Use_symbolics:
		...
		JWKB["name"] = JWKB["name"] + " (numeric GL quad, maxiter=" + str(quad_maxiter) + ")"
		def w1_vec(t):
			Ap2_ = Aprime2_num(t)
		...
		def Mf(t0, t):
			...
			for i in range(Ndim):
				...
				M12[i,i] = S/w10[i]
			P_num = np.linalg.inv(Pinv_num(t))
			M_ = P_num @ (M11 @ Pinv_num(t0) + M12 @ (dPinv(t0) + Pinv_num(t0) @ A_num(t0)))
			return M_
	return Mf
...
def Jordan_WKB_num():
	...
	Pinv = P_sym.inv()
	# numerics ##############
	dPinv = sym.lambdify((ts0), sym.diff(Pinv, ts0), modules=array2mat_c)
	Pinv = sym.lambdify((ts0), Pinv, modules=array2mat_c)
	...
	def Mf(t0, t):
		...
		for i in range(Ndim):
			...
			M11[i,i] = C + S*dw10[i]/(2*(w10[i])**2)
			M12[i,i] = S/w1[i]
		P = np.linalg.inv(Pinv(t))
		M_ = P @ (M11 @ Pinv(t0) + M12 @ (dPinv(t0) + Pinv(t0) @ A_num(t0)))
		return M_
	return Mf
"""
...
def Pseudo_WKB():
	...
	if Use_symbolics:
		...
		M11 = Id.astype(np.complex64)
		M12 = Id.astype(np.complex64)
		PWKB["name"] = PWKB["name"] + " (numeric GL quad, maxiter=" + str(quad_maxiter) + ")"
		def w1_vec(t):
			A_ = Ap(t)
			w1_ = np.ones(Ndim).astype(np.complex64)
			for n in range(Ndim):
...
def Jordan_Magnus1():
	...
	if Use_symbolic:
		...
		def Omega_num(t0, t):
			...
			return Om
		Om1_num = Omega_num
		JM1["name"] = JM1["name"] + " (numeric. GL quad, maxiter=" + str(quad_maxiter) + ")"
...
def Modified_M1():
	...
	if 	Use_symbolic:
		...
		"""
		MM1["name"] = MM1["name"] + " (numeric GL quad, maxiter=" + str(quad_maxiter) + ")"
		def Omega_num(t0, t):
...
def Ext_Pseudo_WKB():
	...
	if Use_symbolics:
		...
		M12 = Id.astype(np.complex64)
		PWKB["name"] = PWKB["name"] + " (numeric GL quad, maxiter=" + str(quad_maxiter) + ")"
		Gamma_num = sym.lambdify((ts0), Gamma, modules=array2mat)
		def gamma_sing(t, n):
			g_ = Gamma_num(t)[n, n]
			return g_
		def w1_vec(t):
		...
		def Mf(t0, t):
			...
			for i in range(Ndim):
				g0 = gamma_sing(t0, i)
				Int_gamma = quad(gamma_sing, t0, t, args=(i), maxiter=quad_maxiter)
				Int_w1 = c_quad(w1_sing, t0, t, ARGS=(i), MAXITER=quad_maxiter)
				C = np.exp(-0.5*Int_gamma)*np.cos(Int_w1)*csqrt(w10[i]/w1[i])
				S = np.exp(-0.5*Int_gamma)*np.sin(Int_w1)*csqrt(w10[i]/w1[i])
				M11[i,i] = C + S*(g0/(2*w10[i]) + dw10[i]/(2*(w10[i])**2))
				M12[i,i] = S/w10[i]
			M_ = (M11 + M12 @ A_num(t0))
			return M_
	return Mf
