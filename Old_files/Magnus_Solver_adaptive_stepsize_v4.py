...
def Jordan_WKB():
	...
	for i in range(0, Ndim):
		w2 = -J[i,i]
		w1 = sym.sqrt(w2)
		C = sym.cos(sym.integrate(w1.subs(ts0, ts1), (ts1, ts0, ts)))/sym.sqrt(w1.subs(ts0, ts))
		S = sym.sin(sym.integrate(w1.subs(ts0, ts1), (ts1, ts0, ts)))/sym.sqrt(w1.subs(ts0, ts))
		dw1 = sym.diff(w1, ts0)
		M11[i,i] = C
		M12[i,i] = S*dw1/(2*w1**2)
	dP = sym.diff(P, ts0)
...
def Magnus_Integrator(t_start, t_stop, h0, x0, A, alpha, type_, order):
	...
	if type_ == "Magnus":
		...
		def M(time, hstep):
			M_ = WKB_matrix_exact(time, time+hstep)
			return M_
	#
	while t <= t_stop:
		x_n = x_[n,:].reshape(Ndim, 1)
