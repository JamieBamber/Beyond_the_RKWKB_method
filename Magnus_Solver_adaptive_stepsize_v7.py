...
PhotonBaryon["alpha"] = 0.1 #6371

PhotonBaryon["eta_0"] = 0.1
PhotonBaryon["k"] = 200 #10

PhotonBaryon["Og0"] = 0.5 
PhotonBaryon["Ob0"] = 0.5

def Apb(k, Og0, Ob0, alpha):
	A = sym.Matrix(
...
A_sym_pb = Apb(PhotonBaryon["k"], PhotonBaryon["Og0"], PhotonBaryon["Ob0"], PhotonBaryon["alpha"])
...
PhotonBaryon["x0"] = np.array([1, 0.1, 1, 0.1, 1])

def rs(t):
	Og0 = PhotonBaryon["Og0"]
	Ob0 = PhotonBaryon["Ob0"]
...
PhotonBaryon["t_start"] = PhotonBaryon["eta_0"]
...
def Apcdm(k, Og0, Ob0):
	A = sym.Matrix(
		[[-2*Og0/ts, -k, -Ob0/2, 0, -ts*(k**2)/3 - 1/ts], 
		 [k/3, 0, 0, 0, k/3],
		 [-6*Og0/ts, 0, -3*Ob0/2, -sym.I*k, -ts*k**2 - 3/ts],
		 [0, 0, 0, 0 - 1/(ts**2), -sym.I*k],
		[-2*Og0/ts, 0, -Ob0/2, 0, -ts*(k**2)/3 - 1/ts]])
	return A
	
A_sym_pcdm = Apcdm(k, Og0, Ob0)
	
A_num_pcdm = sym.lambdify((ts), A_sym_pcdm, modules=array2mat_c)
	
PhotonCDM = {}
PhotonCDM["x0"] = np.array([1, 0, 1, 0, 1])
...
PhotonCDM["t_stop"] = 0.8
PhotonCDM["ylim"] = (-0.75, 1)
	
PhotonCDM["Theta0_sol_coeff"] = [0, 1]
PhotonCDM["Phi_sol_coeff"] = [1, 0]
	
def PhotonCDM_Theta0_sol(t, a, b):
	# define the solution for Theta0
	C = a*(-np.sin(k*(t+b)/np.sqrt(3)))
	return C
	
dx0 = PhotonCDM["A_num"](PhotonCDM["t_start"]) @ PhotonCDM["x0"].reshape(5, 1)

def PhotonCDM_Phi_sol(t, a, b):
	# define the solution for Theta0
	#j1 = special.spherical_jn(1,k*(t+b)/np.sqrt(3))
	#y1 = special.spherical_yn(1,k*(t+b)/np.sqrt(3))
	#C = a*j1
	C = a*(np.sin(k*(t+b)/np.sqrt(3)) - (k*(t+b)/np.sqrt(3))*np.cos(k*(t+b)/np.sqrt(3)))/((k*(t+b)/np.sqrt(3))**3)
	return C
...
def plot_graph():
	...
	ax0.plot(t_vec0, x_true[:,Index], color="k", linewidth=1, linestyle="--", label="true soln (fitted to data)")
